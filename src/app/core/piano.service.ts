...
export class PianoService {
  ...
  getAccidentials(signature) {
    return this.accidentials[signature[1]].slice(0, signature[0]);
  }

  getNote(noteId: string, signature?): PianoNote {
    const accidentials = signature ? this.getAccidentials(signature) : [];
    if (accidentials.indexOf(noteId[0]) !== -1) {
      noteId += signature[1];
    }
  ...
  isNatural(key) {
    return key.reduce((natural, key) => {
      return natural || key.length === 2;
    }, false);
  }

  getNaturalNotes(lowestKey = 16, highestKey = 64, accidentials = true, enharmonics = false): string[] {
    return Object.keys(this.pianoKeyMap).reduce((notes, index) => {
      if (parseInt(index) >= lowestKey && parseInt(index) <= highestKey) {
        if (accidentials && (!this.isNatural(this.pianoKeyMap[index]) || enharmonics)) {
          notes = notes.concat(this.pianoKeyMap[index]);
        } else if (this.isNatural(this.pianoKeyMap[index])) {
          notes.push(this.pianoKeyMap[index][0]);
        }
      }
      return notes;
    }, []);
